...
export function rateLimitMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const limit = hasAuth ? MAX_REQUESTS : UNAUTH_MAX_REQUESTS;
  const key = hasAuth ? `auth:${ip}` : `unauth:${ip}`;
  const now = Date.now();
  ...
  if (bucket.count > limit) {
    ...
    res.status(429).json({ error: `Too many requests. Limit: ${limit} per minute.` });
    return;
  }
  next();
}
