...
export class PolicyEngine {
    ...
    _partitionAndMatch(rules, request, ctx) {
        ...
        for (let i = 0; i < rules.length; i++) {
            const rule = rules[i];
            if (rule.action === 'monitor')
                continue; // skip in terminal pass
            if (this._evalConditions(rule, request, ctx)) {
    ...
    _computeRiskScore(result, monitorBoost, ctx) {
        ...
        const multiplier = riskTier === 'critical'
            ? 3.0
            : riskTier === 'high'
                ? 2.0
                : riskTier === 'medium'
    ...
    static fingerprintBundle(bundle) {
        const content = JSON.stringify(bundle.rules, Object.keys(bundle.rules).sort());
...
export class PolicyCompiler {
     ...
     */
    static compile(doc) {
        const compiledRules = doc.rules.map((rule) => PolicyCompiler.compileRule(rule));
...
export function evalToolCondition(condition, toolName) {
    // "in" — exact match list
...
export function evalParamConditions(conditions, params) {
...
export function evalValueConstraint(constraint, value) {
...
export function evalTimeWindow(tw) {
    const now = new Date();
    if (tw.within) {
