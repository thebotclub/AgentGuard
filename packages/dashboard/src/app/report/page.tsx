  ...
  return (
    <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
      <td style={{ padding: '10px 12px', fontSize: '13px' }}>
        <span style={{ fontFamily: 'monospace', fontWeight: 600, color: '#475569' }}>{control.id}</span>
      </td>
      <td style={{ padding: '10px 12px', fontSize: '13px', color: '#0f172a' }}>{control.name}</td>
    ...
    <div id="report-content">
      ...
      <div style={{
        ...
        {[
          ...
          <div key={stat.label} style={{ padding: '16px 20px', background: '#fff', textAlign: 'center' }}>
      ...
      {/* OWASP Controls */}
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderTop: 'none', padding: '24px' }}>
          ...
          <thead>
            <tr style={{ background: '#f8fafc' }}>
              {['Control', 'Description', 'Score', 'Status'].map((h) => (
                <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: '11px', fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            <tbody>
              {report.policies.map((p) => (
                <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '10px 12px', fontSize: '13px', fontWeight: 500, color: '#0f172a' }}>{p.name}</td>
                  <td style={{ padding: '10px 12px', fontSize: '13px', fontFamily: 'monospace', color: '#475569' }}>{p.activeVersion ? `v${p.activeVersion}` : '—'}</td>
                  <td style={{ padding: '10px 12px', fontSize: '13px', color: p.defaultAction === 'BLOCK' ? '#dc2626' : '#16a34a' }}>{p.defaultAction}</td>
                  <td style={{ padding: '10px 12px', fontSize: '12px', color: '#64748b' }}>{format(new Date(p.updatedAt), 'PP')}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderTop: 'none', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
          {[
            ...
            <div key={s.label} style={{ padding: '10px 16px', background: '#f8fafc', borderRadius: '8px', fontSize: '13px' }}>
            ...
            <tbody>
              {report.agentHealth.agents.slice(0, 10).map((a) => (
                <tr key={a.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '8px 12px', fontSize: '13px', color: '#0f172a' }}>{a.name}</td>
                  <td style={{ padding: '8px 12px', fontSize: '12px', color: a.status === 'ACTIVE' ? '#16a34a' : '#dc2626' }}>{a.status}</td>
                  <td style={{ padding: '8px 12px', fontSize: '12px', color: '#64748b' }}>{a.riskTier}</td>
                  <td style={{ padding: '8px 12px', fontSize: '12px', color: '#64748b' }}>{a.framework ?? '—'}</td>
    ...
    <div>
        ...
        <div style={{
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  background: '#f8fafc', cursor: 'pointer', fontSize: '12px', color: '#475569',
                }}
              >{r.label}</button>
            ))}
          </div>
          ...
          {report && (
            <button
              ...
              style={{
                padding: '9px 24px', borderRadius: '8px', border: '1px solid #e2e8f0',
      ...
      {!generate && !report && (
        <div style={{
          ...
          <h3 style={{ margin: '0 0 8px', color: '#0f172a', fontSize: '18px' }}>Generate Your Compliance Report</h3>
