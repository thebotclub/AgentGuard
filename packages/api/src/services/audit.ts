...
import type { AuditEvent, ActionType, PolicyDecision, RiskTier } from '@prisma/client';
...
export class AuditService extends BaseService {
  ...
  async queryEvents(query: QueryAuditEventsInput): Promise<AuditEvent[]> {
    return this.db.auditEvent.findMany({
      where: {
        ...this.tenantScope(),
        ...(query.agentId ? { agentId: query.agentId } : {}),
        ...(query.sessionId ? { sessionId: query.sessionId } : {}),
        ...(query.decision ? { policyDecision: query.decision as PolicyDecision } : {}),
        ...(query.riskTier ? { riskTier: query.riskTier as RiskTier } : {}),
  ...
  async getEvent(eventId: string): Promise<AuditEvent> {
    ...
    if (!event) throw new NotFoundError('AuditEvent', eventId);
    return event;
  }
   ...
   */
  async ingestBatch(batch: TelemetryBatchInput): Promise<IngestResult> {
    let accepted = 0;
    let rejected = 0;
    const errors: Array<{ clientEventId: string; reason: string }> = [];

    // Group events by sessionId for sequential processing
    const bySession = new Map<string, typeof batch.events>();
   ...
   */
  private async ingestSingleEvent(
    agentId: string,
    sessionId: string,
    event: TelemetryBatchInput['events'][number],
  ): Promise<void> {
    await this.withTransaction(async (tx) => {
      ...
      const riskTier = riskTierLabel as RiskTier;
      ...
      await tx.auditEvent.create({
        data: {
          ...
          actionParams: event.actionParams
            ? (event.actionParams as import('@prisma/client').Prisma.InputJsonValue)
            : undefined,
...
export function computeEventHash(previousHash: string, event: HashableEventPayload): string {
...
function mapDecision(decision: string): PolicyDecision {
  const normalized = decision.toUpperCase();
  const valid = [
    'ALLOW', 'BLOCK', 'MONITOR', 'HITL_PENDING',
    'HITL_APPROVED', 'HITL_REJECTED', 'HITL_TIMEOUT', 'KILLED', 'ERROR',
  ];
  if (valid.includes(normalized)) return normalized as PolicyDecision;
...
function mapActionType(actionType: string): ActionType {
  ...
  if (valid.includes(normalized)) return normalized as ActionType;
  return 'TOOL_CALL';
}
