...
import { PolicyDocumentSchema, BASE_RISK_SCORES, POLICY_BUNDLE_TTL_SECONDS, } from '@agentguard/shared';
...
export class PolicyService extends BaseService {
    ...
    async activateVersion(policyId, version) {
        ...
        if (!pv)
            throw new NotFoundError('PolicyVersion', targetVersion);
        // Cache the bundle in Redis with TTL
        const bundle = pv.compiledBundle;
        await this.cacheBundle(policyId, bundle);
    ...
    async getBundleForAgent(agentId) {
        ...
        if (!policy || !policy.activeVersion)
            return null;
        const pv = await this.db.policyVersion.findUnique({
     ...
     */
    async testPolicy(policyId, tests) {
    ...
    _evalConditions(rule, request, ctx) {
        ...
        for (const dataMap of rule.dataClassConditions) {
            ...
            if (!evalParamConditions(dataMap, dataCtx))
                return false;
        }
        for (const tw of rule.timeConditions) {
            if (!evalTimeWindow(tw))
                return false;
        }
        return true;
    }
    _partitionAndMatch(rules, request, ctx) {
    ...
    _checkRateLimit(rule, request, ctx) {
    ...
    _computeRiskScore(result, monitorBoost, ctx) {
        const base = BASE_RISK_SCORES[result];
        const riskTier = ctx.sessionContext?.['riskTier'] ?? 'medium';
        const multiplier = riskTier === 'critical' ? 3.0 : riskTier === 'high' ? 2.0 : riskTier === 'medium' ? 1.5 : 1.0;
        return Math.min(1000, Math.round((base + monitorBoost) * multiplier));
    }
    _mostRestrictive(a, b) {
        const rank = { block: 3, require_approval: 2, monitor: 1, allow: 0 };
        return (rank[a.action] ?? 0) >= (rank[b.action] ?? 0) ? a : b;
    }
    _buildReason(rule, result) {
        switch (result) {
    ...
    async persistVersion(tx, policyId, doc, bundle, input) {
        const compiledBundle = bundle;
    ...
    async cacheBundle(policyId, bundle) {
        await this.redis.set(RedisKeys.policyBundle(this.tenantId, policyId), JSON.stringify(bundle), 'EX', POLICY_BUNDLE_TTL_SECONDS);
    }
}
// ─── PolicyCompilerService (ported from SDK) ──────────────────────────────────
export class PolicyCompilerService {
    static compile(doc, policyId, tenantId) {
        const compiledRules = doc.rules.map((rule) => PolicyCompilerService.compileRule(rule));
        // Extract prompt injection check config from checks array (if present)
        const promptInjectionConfig = doc.checks?.find((c) => c.type === 'prompt_injection');
        const toolIndex = {};
        for (let i = 0; i < compiledRules.length; i++) {
            ...
            if (!tc) {
                if (!toolIndex['__no_tool__'])
                    toolIndex['__no_tool__'] = [];
    ...
    static compileRule(rule) {
        ...
        for (const when of rule.when) {
            if ('tool' in when) {
                ...
                paramConditions.push(when.params);
            }
            else if ('context' in when) {
                contextConditions.push(when.context);
            }
            else if ('dataClass' in when) {
                dataClassConditions.push(when.dataClass);
            }
...
function evalToolCondition(condition, toolName) {
    if (condition.in !== undefined && condition.in.length > 0) {
        if (!condition.in.includes(toolName))
            return false;
    }
    if (condition.not_in !== undefined && condition.not_in.length > 0) {
        if (condition.not_in.includes(toolName))
            return false;
    }
    if (condition.matches !== undefined && condition.matches.length > 0) {
        if (!micromatch.isMatch(toolName, condition.matches))
            return false;
    }
...
function evalParamConditions(conditions, params) {
...
function evalValueConstraint(constraint, value) {
    ...
    if ('not_in' in constraint && constraint.not_in !== undefined) {
        if (constraint.not_in.includes(value))
            return false;
    }
    if ('contains' in constraint && constraint.contains !== undefined) {
        if (!String(value).includes(constraint.contains))
            return false;
    }
    if ('contains_any' in constraint && constraint.contains_any !== undefined) {
        const str = String(value);
        if (!constraint.contains_any.some((s) => str.includes(s)))
            return false;
    }
    if ('pattern' in constraint && constraint.pattern !== undefined) {
        if (!micromatch.isMatch(String(value), constraint.pattern))
            return false;
    }
    ...
    if ('domain_not_in' in constraint && constraint.domain_not_in !== undefined) {
        ...
        if (constraint.domain_not_in.includes(domain))
            return false;
    }
    if ('exists' in constraint && constraint.exists !== undefined) {
        if (constraint.exists !== (value !== undefined && value !== null))
            return false;
    }
    if ('is_null' in constraint && constraint.is_null !== undefined) {
        const isNull = value === null || value === undefined;
        if (constraint.is_null !== isNull)
            return false;
    }
    return true;
}
function evalTimeWindow(tw) {
    const now = new Date();
    if (tw.within) {
