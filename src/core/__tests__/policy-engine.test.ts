...
import { PolicyEngine, PolicyCompiler, evalToolCondition, evalValueConstraint } from '@/core/policy-engine.js';
...
function makeRequest(tool: string, extra?: Partial<ActionRequest>): ActionRequest {
  return {
    id: randomUUID(),
    agentId: 'agent-test-001',
    tool,
...
describe('PolicyEngine', () => {
  ...
  it('rate limit blocks when call count exceeds window limit on rate-limited allow rule', () => {
    const rateLimitDoc: PolicyDocument = {
      ...BASE_DOC,
...
describe('PolicyError', () => {
  ...
  it('PolicyError.toJSON serialises all fields', () => {
    ...
    expect((json['details'] as { tool: string })['tool']).toBe('admin:delete');
